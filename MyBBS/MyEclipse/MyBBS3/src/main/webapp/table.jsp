			...
			<div class="span12">
				<h3 class="heading">快来灌水</h3>
					...
					<div class="alert alert-error">
						...
						<strong>
						操作信息: ${requestScope.msg !=null ? requestScope.msg : "" }
						快来和小伙伴们一起灌水吧！
						</strong>
				...
				<table class="table table-bordered table-striped table_vam"
					...
					<c:forEach var="article" items="${pageBean.data}" varStatus="status">
						
								<tr>
									<td>  ${status.count} </td>
									<td>
										<a href="upload/.jpg" title="" class="cbox_single thumbnail">
											<%--请求得到用户的头像 --%>
											<img src="UserControl?action=pic&id=${article.user.id}"
									...
									<td>
										<!-- 没登陆，游客 uid=0 -->
											...
											<c:if test="${sessionScope.user.id!=null && sessionScope.user.id==article.user.id }" >
												<a href="ArticleControl?action=del&id=${article.id}"
													title="删除本帖">

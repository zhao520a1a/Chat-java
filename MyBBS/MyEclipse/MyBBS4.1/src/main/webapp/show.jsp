...
//在model上显示回帖信息和设置删除回帖权限
		...
		*/
		function showhuitie(rootid,uid,root_uid,data){
			 $('#tab_head').empty();
			  $('#tab_content').empty();
			  $('#tab_foot').empty();
			  var head="";
...
//如果采用了通过后台数据库查询主贴的标题，要采用以下方式：list是返回的一个帖子的回帖列表
...
//显示回帖的AJAS
		function rshow(rootid,uid,root_uid) {
			//rootid:主贴 id  uid:当前浏览用户id（两种：游客和登录的用户）  root_uid:主帖用户的id 
			...
			//把该主帖子的id值付给content.jsp model下的隐藏字段，代表该贴是从贴
			 ...
			 $.ajax({
				  ...
				  success : function(data){	
					 ...
					 showhuitie(rootid,uid,root_uid,data);
				  }		  
	         });

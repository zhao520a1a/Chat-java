...
<div id="rshow" class="modal hide fade">
	...
	<div class="modal-footer">
		...
		<%-- 权限问题： 游客只显示登录按钮，用户显示回帖按钮--%>
		<c:if test="${sessionScope.user == null }">
			<%--游客 --%>
			<a class="btn btn-warning" href="#login" title="登录"
				data-toggle="modal" id="myr" data-backdrop="static" title="登录"
				onclick="javascript:$('#rshow').modal('hide');document.getElementById('submenu').innerHTML='登录'">
				登录 </a>
		</c:if>
		<c:if test="${sessionScope.user != null }">
			<%--用户 --%>

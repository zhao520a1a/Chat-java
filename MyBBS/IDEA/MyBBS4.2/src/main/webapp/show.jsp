...
<body class="sidebar_hidden">
						...
						<ul class="nav user_menu pull-right">
							...
							<li class="dropdown"><a href="#" class="dropdown-toggle"
								data-toggle="dropdown"> <c:if
										...
										游客
								</c:if> <%--显示用户头像和用户名 --%> <c:if test="${sessionScope.user != null}">
										<img src="UserControl?action=pic&id=${sessionScope.user.id}"
											alt="请登录" class="user_avatar" /> 
	...
	<script type="text/javascript">
		$(function() {
			var username = $.cookie('http://www.bbs.com/username'); // 获得cookie
			var password = $.cookie('http://www.bbs.com/password');
...
//删除从贴
 ...
 //新帖使用正常表单提交，回帖使用ajax异步,否则不能定位到model
		   if($("#postrootaction").val()==='addReply'){ 
			   $.ajax({
				   type: "post",
					  url: "ArticleControl",	
					  data:"action=addReply&title="+$('#title').val()+"&content="+txt+"&rootid="+$("#postrootid").val()+"&userid="+$('#userid').val(),
					  dataType: "text json",
					  success : function(data){	
...
//执行发从贴窗体
		function addc() {
			//将content.jsp中隐藏域中的属性postrootaction的值改为addReply,好对应ArticleControl中case "addReply"
			$("#postrootaction").val("addReply");
			$('#rshow').modal('hide');  //将rshow隐藏
			$('#title').val("");        //清空标题输入框中的内容
			CKEDITOR.instances.content.setData(' ');//清空CKEdite组件中内容输入框中的内容
			$('#post').modal('show');
 
		}

//在model上显示回帖信息和设置删除回帖权限
		/*
			rootid：主贴id 
			uid:当前浏览用户id  
			root_uid:主帖的用户id
		...
		*/
		function showhuitie(rootid,uid,root_uid,data,roottitle){
			 $('#tab_head').empty();
			  $('#tab_content').empty();
			  $('#tab_foot').empty();
			  var head="";
...
//如果采用了通过前端页面之间传递数据，来获得主贴的标题，要采用以下方式:
			     ...
			     //增加删除重贴按钮,浏览用户和主帖用户相同的情况下，才能删除
			     if(uid===root_uid){//字符串是否相等,删除从贴
			    	 footer = "<div id='foot "+i+"' > "+			    	    
			    		"	<a class='btn btn-danger'  href='#' "+
			     			"onclick='del("+data[i].id+","+data[i].rootid+")'>删除 </a>"+   
			     			"</div> ";
			     }
			   //增加帖子的标题，在tab上
			     if(i == 0){
			    	 //标记上第一个值能够被激活显示
			    	 head=head+				     
				      "<li class='active' ><a href='#tab_l"+i+"' data-toggle='tab'>"+data[i].title+"</a></li>";
			     }else{
			    	 head=head+				     
				      "<li><a href='#tab_l"+i+"' data-toggle='tab'>"+data[i].title+"</a></li>";
			     }
			     
			    //增加帖子的内容，在tab上
			    if(i==0){
			    	 content=content+"<div class='tab-pane active' id='tab_l"+i+"'> "+footer+" <br/> <p>"+data[i].content+"</p>"+" </div>";
			    }else{
			    	 content=content+"<div class='tab-pane' id='tab_l"+i+"'> "+footer+" <br/> <p>"+data[i].content+"</p>"+" </div>";
			    }
				$('#span_title').html(roottitle);//添加主贴标题
			  });
...
/* 
 显示回帖的AJAS
rootid:主贴 id  
uid:当前浏览用户id（两种：游客和登录的用户）  
root_uid:主帖用户的id 
roottitle:主贴标题
*/
		function rshow(rootid,uid,root_uid,roottitle) {
			
			 //把主帖子的id值付给rshow.jsp model下的隐藏字段
			  $("#rshowid").val(rootid);
			//把该主帖子的id值付给content.jsp model下的隐藏字段，代表该贴是从贴
			  $("#postrootid").val(rootid);
			
			 $.ajax({
				  type: "post",
				  url: "ArticleControl",	
				  data:"action=showReply&rootid="+rootid,  // action为showReply 到ArticleControl中   
				  dataType: "text json",
				  success : function(data){	
					  //显示回帖内容，并且显示在model窗体中
					 showhuitie(rootid,uid,root_uid,data,roottitle);
				  }		  
	         });
